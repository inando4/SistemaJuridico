...
/**
 * Administracion del catalogo de estados procesales.
 *
 * <p><b>Deshabilitar y borrar no son lo mismo.</b> Deshabilitar retira el estado
 * de las opciones nuevas pero lo conserva donde ya se uso; borrar solo se permite
 * si nunca lo uso nadie. Un estado que aparece en el historial no puede
 * desaparecer, o ese historial dejaria de poder explicarse.
 */
@Service
public class ProceduralStatusService {

    private final ProceduralStatusRepository catalogo;
    private final AuditRecorder auditoria;
    private final Clock clock;

    public ProceduralStatusService(ProceduralStatusRepository catalogo,
    ...
    public Optional<String> crear(String nombre, String descripcion, CuentaActual jefa) {
        ...
        auditoria.registrar("PROCEDURAL_STATUS", id, "CREATE", jefa.id(), jefa.id(), null,
    ...
    public void cambiarDisponibilidad(UUID id, boolean habilitado, long version, CuentaActual jefa) {
        ...
        auditoria.registrar("PROCEDURAL_STATUS", id, "AVAILABILITY", jefa.id(), jefa.id(),
    ...
    public Optional<String> eliminar(UUID id, long version, CuentaActual jefa) {
        ...
        if (catalogo.enUsoActual(id)) {
            return Optional.of("Este estado esta en uso por algun expediente. "
                    + "Puede deshabilitarlo para que no se ofrezca en adelante.");
        }
        if (catalogo.enUsoHistorico(id)) {
            return Optional.of("Este estado aparece en el historial de algun expediente y no "
                    + "puede borrarse. Puede deshabilitarlo.");
        }

        auditoria.registrar("PROCEDURAL_STATUS", id, "DELETE", jefa.id(), jefa.id(),

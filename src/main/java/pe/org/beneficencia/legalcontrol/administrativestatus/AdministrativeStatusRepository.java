...
/**
 * Catalogo de estados de procedimientos administrativos.
 *
 * <p>Distinto del catalogo judicial y del de pendientes. Arranca vacio: los
 * valores los carga la jefatura desde la aplicacion.
 */
@Repository
public class AdministrativeStatusRepository {

    private final JdbcClient jdbc;

    public AdministrativeStatusRepository(JdbcClient jdbc) {
        this.jdbc = jdbc;
    }
    ...
    public List<Map<String, Object>> todos() {
        return jdbc.sql("""
                ...
                FROM administrative_status ORDER BY lower(btrim(name))
                """).query().listOfRows();
    }
    ...
    public List<Map<String, Object>> habilitados() {
        return jdbc.sql("""
                SELECT id, name FROM administrative_status
    ...
    public Optional<Map<String, Object>> porId(UUID id) {
        return jdbc.sql("""
                SELECT id, name, description, enabled, version
                FROM administrative_status WHERE id = :id
    ...
    public UUID insertar(String nombre, String descripcion, UUID actor, Instant ahora) {
        ...
        jdbc.sql("""
                INSERT INTO administrative_status (id, name, description, enabled, created_by,
    ...
    public boolean eliminar(UUID id, long version) {
        return jdbc.sql("DELETE FROM administrative_status WHERE id = :id AND version = :version")
                .param("id", id).param("version", version).update() == 1;
    }

    public boolean nombreYaUsado(String nombre, UUID excepto) {
        Integer total = jdbc.sql("""
                SELECT count(*) FROM administrative_status
    ...
    public boolean enUsoActual(UUID id) {
        Integer total = jdbc.sql("""
                SELECT count(*) FROM administrative_procedure WHERE administrative_status_id = :id
                """).param("id", id).query(Integer.class).single();
        return total != null && total > 0;
    }

    /**
     * ¿Lo uso alguna vez algun procedimiento, aunque ya no?
     *
     * <p>Si se borrara, el historial que lo menciona quedaria apuntando a un estado
     * inexistente y dejaria de poder explicarse.
     */
    public boolean enUsoHistorico(UUID id) {
        Integer total = jdbc.sql("""
                SELECT count(*) FROM procedure_history_status_reference
                WHERE administrative_status_id = :id
                """).param("id", id).query(Integer.class).single();
        return total != null && total > 0;
    }
}
